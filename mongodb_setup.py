"""
mongodb_setup.py — MongoDB Schema Validation, Indexing, and Database Seeding Script.
...
to replicate the original SQLite database state.
"""
import os
import sys
...
# Add current dir to path to import auth helpers
...
def create_validations_and_indexes(db):
    ...
    # Unique sparse index for mssv (allows multiple nulls for teachers/admins but unique for students)
    ...
    if "exam_rooms" in db.list_collection_names():
    ...
    if "exam_sessions" in db.list_collection_names():
    ...
    if "violation_logs" in db.list_collection_names():
    ...
    # ------------------------------------------------------------------
    # Collection: system_settings (Supplemental)
    # ------------------------------------------------------------------
...
def seed_database(db):
        ...
        "students": [
            {
                ...
                "face_embedding": None,  # Will be populated once student uploads a photo
    ...
    room101_id = db["exam_rooms"].insert_one(room101_doc).inserted_id
    print(f"  ➕ Đã thêm phòng thi ROOM_101 và ghi danh '{student_doc['full_name']}'")
    ...
    room102_id = db["exam_rooms"].insert_one(room102_doc).inserted_id
    print(f"  ➕ Đã thêm phòng thi trống ROOM_102")
...
def main():
    ...
    try:
        ...
        create_validations_and_indexes(db)
        seed_database(db)
